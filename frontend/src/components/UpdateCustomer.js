...
function UpdateCustomer() {
    ...
    useEffect(() => {
        axios.get("http://localhost:8082/customer/" + userId).then(resp => setCustomer({
    ...
    const handleSubmit = (event) => {
        ...
        if (noErrors) {
            axios.put("http://localhost:8082/customer/update/" + payload.userId, payload).then(resp =>{
                navigate("/",{replace:true});
                alert('Customer updated with id: ' + resp.data.userId);});
        }
    }
